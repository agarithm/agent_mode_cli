...
def build_internal_system_prompt(agent_name: str) -> str:
    ...
    return (
        ...
        "You have file and system tools including: list_dir (safe directory listing), read_file (safe file reading), search_files (safe ripgrep search), python_exec (run short Python snippets; returns JSON), edit_file (file editing: structured edits and overwrite/append), bash (execute local shell commands). "
        ...
        "Make use of these tools to accomplish user tasks. "
        "You have web browsing tools including: http_fetch (HTTP fetch; mode='simple' for no-JS and mode='browser' for JS via Playwright/Chromium). "
        "Use http_fetch to retrieve web pages and APIs as needed to accomplish user tasks or to lookup current information. "
